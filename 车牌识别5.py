...
class LicensePlateApp(QMainWindow):
	...
	def create_right_panel(self):
		...
		self.show_bbox_check.setChecked(True)  # 默认勾选
		self.show_conf_check = QCheckBox("显示置信度")
		self.show_conf_check.setChecked(True)  # 默认勾选
        ...
        """
		self.show_bbox_check.setStyleSheet(checkbox_style)
		self.show_conf_check.setStyleSheet(checkbox_style)
	...
	def update_frame(self, frame, detections):
		...
		# 绘制YOLO检测框 - 默认显示，不依赖复选框状态
		show_bbox = True  # 默认显示
		show_conf = True  # 默认显示

		# 如果复选框有效，则使用其状态
		if self.ui_accessor.is_valid('show_bbox_check'):
			show_bbox = self.show_bbox_check.isChecked()
		if self.ui_accessor.is_valid('show_conf_check'):
